...
export default function BestUnder40kPage() {
  const cars = sortByPopularity(vehicleDatabase.filter(v => v.price <= 40));
  const sedans = sortByPopularity(cars.filter(v => v.bodyType === 'sedan'));
  const suvs = sortByPopularity(cars.filter(v => v.bodyType === 'suv' || v.bodyType === 'crossover'));
  const trucks = sortByPopularity(cars.filter(v => v.bodyType === 'truck'));
  const hybrids = sortByPopularity(cars.filter(v => v.powertrain === 'hybrid' || v.powertrain === 'phev'));
