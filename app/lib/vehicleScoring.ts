...
export function scoreVehicles(preferences: UserPreferences): ScoredVehicle[] {
  ...
  for (const vehicle of finalCandidates) {
    ...
    // --- SEGMENT BONUS (Niceness factor) ---
    if (vehicle.segment === 'premium') score += 8;
    if (vehicle.segment === 'luxury') score += 15;
    if (vehicle.segment === 'exotic') score += 20;
    
    // --- PRICE-BASED NICENESS (within budget, higher = nicer) ---
    // Vehicles closer to budget max get a bonus (they're typically nicer)
    const priceRatio = vehicle.price / budgetRange.max;
    if (priceRatio >= 0.7 && priceRatio <= 1.0) {
      score += 10; // Sweet spot - nice car within budget
    } else if (priceRatio >= 0.5 && priceRatio < 0.7) {
      score += 5;
    }
    
    // --- PREMIUM BRAND BONUS ---
    const premiumBrands = ['BMW', 'Mercedes-Benz', 'Audi', 'Lexus', 'Porsche', 'Genesis', 'Volvo', 'Acura', 'Infiniti', 'Lincoln', 'Cadillac'];
    const luxuryBrands = ['Porsche', 'Land Rover', 'Jaguar', 'Maserati', 'Alfa Romeo'];
    const exoticBrands = ['Ferrari', 'Lamborghini', 'McLaren', 'Aston Martin', 'Bentley', 'Rolls-Royce', 'Bugatti', 'Koenigsegg', 'Pagani', 'Maserati', 'Lotus', 'Lucid'];
    
    if (exoticBrands.includes(vehicle.brand)) {
      score += 15;
    } else if (luxuryBrands.includes(vehicle.brand)) {
      score += 10;
    } else if (premiumBrands.includes(vehicle.brand)) {
      score += 5;
    }
  ...
  // Sort by score descending, then by price descending (nicer cars first when tied)
  scoredVehicles.sort((a, b) => {
    if (b.score !== a.score) {
      return b.score - a.score;
    }
    // If scores are equal, show higher-priced (nicer) vehicle first
