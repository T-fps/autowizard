...
// Car image URL generator using Imagin Studio API
function getCarImageUrl(vehicle: Vehicle): string {
  const make = vehicle.brand.toLowerCase().replace(/\s+/g, '-').replace('mercedes-benz', 'mercedes');
  
  // Extract model family from vehicle name
  let modelFamily = vehicle.name
  ...
  // Special cases for model family names
  const modelMappings: Record<string, string> = {
    // Toyota
    'camry': 'camry', 'corolla': 'corolla', 'rav4': 'rav4', 'highlander': 'highlander',
    '4runner': '4runner', 'tacoma': 'tacoma', 'tundra': 'tundra', 'sienna': 'sienna',
    'prius': 'prius', 'supra': 'supra', 'gr86': '86', 'crown': 'crown', 'sequoia': 'sequoia',
    'land': 'land-cruiser', 'grand': 'grand-highlander', 'bz4x': 'bz4x', 'venza': 'venza',
    // Honda
    'accord': 'accord', 'civic': 'civic', 'cr-v': 'cr-v', 'pilot': 'pilot', 'hr-v': 'hr-v',
    'passport': 'passport', 'odyssey': 'odyssey', 'ridgeline': 'ridgeline',
    // Ford
    'f-150': 'f-150', 'f-250': 'f-250', 'mustang': 'mustang', 'bronco': 'bronco',
    'explorer': 'explorer', 'escape': 'escape', 'edge': 'edge', 'expedition': 'expedition',
    'ranger': 'ranger', 'maverick': 'maverick', 'transit': 'transit',
    // Chevrolet
    'silverado': 'silverado', 'tahoe': 'tahoe', 'suburban': 'suburban', 'traverse': 'traverse',
    'equinox': 'equinox', 'colorado': 'colorado', 'camaro': 'camaro', 'corvette': 'corvette',
    'bolt': 'bolt', 'blazer': 'blazer', 'trailblazer': 'trailblazer', 'malibu': 'malibu',
    // BMW
    '3': '3-series', '4': '4-series', '5': '5-series', '7': '7-series',
    'x1': 'x1', 'x3': 'x3', 'x5': 'x5', 'x7': 'x7', 'z4': 'z4',
    'i4': 'i4', 'ix': 'ix', 'm3': 'm3', 'm4': 'm4', 'm5': 'm5',
    // Mercedes
    'c-class': 'c-class', 'e-class': 'e-class', 's-class': 's-class',
    'glc': 'glc', 'gle': 'gle', 'gls': 'gls', 'g-class': 'g-class',
    'eqs': 'eqs', 'eqb': 'eqb', 'amg': 'amg-gt',
    // Audi
    ...
    'taycan': 'taycan', '718': '718',
    // Tesla
    'model': vehicle.name.includes('Model 3') ? 'model-3' : 
             vehicle.name.includes('Model Y') ? 'model-y' :
             vehicle.name.includes('Model S') ? 'model-s' :
             vehicle.name.includes('Model X') ? 'model-x' : 'cybertruck',
    // Exotic brands
    'urus': 'urus', 'huracan': 'huracan', 'revuelto': 'revuelto',
    'roma': 'roma', '296': '296', 'sf90': 'sf90', 'purosangue': 'purosangue', 'f8': 'f8', '812': '812',
    'artura': 'artura', 'gt': 'gt', '750s': '750s', '720s': '720s',
    'vantage': 'vantage', 'db12': 'db12', 'db11': 'db11', 'dbs': 'dbs', 'dbx': 'dbx',
    'continental': 'continental', 'flying': 'flying-spur', 'bentayga': 'bentayga',
    'ghost': 'ghost', 'phantom': 'phantom', 'cullinan': 'cullinan', 'spectre': 'spectre',
    'ghibli': 'ghibli', 'quattroporte': 'quattroporte', 'grecale': 'grecale', 'granturismo': 'granturismo', 'mc20': 'mc20',
    'emira': 'emira', 'eletre': 'eletre', 'emeya': 'emeya',
    'air': 'air', 'gravity': 'gravity',
    'chiron': 'chiron', 'tourbillon': 'tourbillon',
  };

  // Try to find a mapping
  for (const [key, value] of Object.entries(modelMappings)) {
    if (modelFamily.includes(key) || vehicle.name.toLowerCase().includes(key)) {
      modelFamily = value;
      break;
    }
  }
  ...
  const paintColors: Record<string, string> = {
    'exotic': 'pspc0014', // Red for exotics
    'luxury': 'pspc0012', // Black for luxury
    'premium': 'pspc0020', // Blue for premium
    'mainstream': 'pspc0029', // Silver for mainstream
    'economy': 'pspc0029', // Silver for economy
  };
  const paintId = paintColors[vehicle.segment] || 'pspc0029';

  return `https://cdn.imagin.studio/getImage?customer=hrjavascript-mastery&make=${make}&modelFamily=${modelFamily}&paintId=${paintId}&angle=01`;
}

// Fallback gradient based on brand
              ...
              <div className="relative">
                <div className={`aspect-video bg-gradient-to-br ${getBrandGradient(vehicle.brand)} rounded-2xl flex items-center justify-center border border-slate-600 overflow-hidden relative`}>
                  <CarImage 
                    ...
                    alt={`${year} ${vehicle.name}`}
                    fallbackEmoji={vehicle.segment === 'exotic' ? '🏎️' : '🚗'}
