      ...
      <div className="relative bg-gradient-to-br from-slate-100 via-slate-50 to-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">
            {vehicles[0].name} <span className="text-amber-600">vs</span> {vehicles[1].name}
          </h1>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {vehicles.map((vehicle) => (
            <div key={vehicle.name} className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-lg transition-shadow">
              ...
              <div className="p-6 bg-white">
                ...
                <p className="text-3xl font-bold text-amber-600 mb-4">{formatPrice(vehicle.price)}</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="divide-y divide-slate-200">
            {/* Price Row */}
            <div className="grid grid-cols-3 p-4 bg-amber-50/50">
              <div className="text-slate-600 font-medium">Starting MSRP</div>
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-bold ${v.price === lowestPrice ? 'text-green-600' : 'text-slate-900'}`}>
                  {formatPrice(v.price)}
            ...
            <div className="grid grid-cols-3 p-4">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-medium ${v.hp === highestHP && vehicles[0].hp !== vehicles[1].hp ? 'text-green-600' : 'text-slate-900'}`}>
                  {v.hp.toLocaleString()} hp
                  {v.hp === highestHP && vehicles[0].hp !== vehicles[1].hp && <span className="ml-2 text-xs">✓ More</span>}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 p-4 bg-slate-50/50">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-medium ${v.mpgCity === highestMPGCity && vehicles[0].mpgCity !== vehicles[1].mpgCity ? 'text-green-600' : 'text-slate-900'}`}>
                  {v.powertrain === 'ev' ? `${v.mpgCity} MPGe` : `${v.mpgCity} mpg`}
                  {v.mpgCity === highestMPGCity && vehicles[0].mpgCity !== vehicles[1].mpgCity && <span className="ml-2 text-xs">✓ Better</span>}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 p-4">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-medium ${v.mpgHighway === highestMPGHwy && vehicles[0].mpgHighway !== vehicles[1].mpgHighway ? 'text-green-600' : 'text-slate-900'}`}>
                  {v.powertrain === 'ev' ? `${v.mpgHighway} MPGe` : `${v.mpgHighway} mpg`}
                  {v.mpgHighway === highestMPGHwy && vehicles[0].mpgHighway !== vehicles[1].mpgHighway && <span className="ml-2 text-xs">✓ Better</span>}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 p-4 bg-amber-50/50">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center ${v.reliability === highestReliability && vehicles[0].reliability !== vehicles[1].reliability ? 'text-green-600' : 'text-slate-900'}`}>
                  ...
                  {v.reliability === highestReliability && vehicles[0].reliability !== vehicles[1].reliability && <span className="text-xs text-green-600">✓ Better</span>}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 p-4 bg-slate-50/50">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-medium ${v.seats === highestSeats && vehicles[0].seats !== vehicles[1].seats ? 'text-green-600' : 'text-slate-900'}`}>
                  {v.seats} passengers
