      ...
      <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-b border-slate-700">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">
            {vehicles[0].name} <span className="text-cyan-400">vs</span> {vehicles[1].name}
          </h1>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {vehicles.map((vehicle) => (
            <div key={vehicle.name} className="bg-slate-900/50 rounded-2xl overflow-hidden border border-slate-700">
              ...
              <div className="p-6">
                ...
                <p className="text-3xl font-bold text-cyan-400 mb-4">{formatPrice(vehicle.price)}</p>
                ...
                <Link href={`/cars/${getVehicleSlug(vehicle.name)}`} className="text-cyan-400 hover:text-cyan-300 font-medium">
                  View Full Details →
                </Link>
              </div>
            </div>
        ...
        <div className="bg-slate-900/50 rounded-2xl border border-slate-700 overflow-hidden">
          <div className="p-6 border-b border-slate-700 bg-slate-800/50">
            <h3 className="text-xl font-bold text-white">Specifications Comparison</h3>
          </div>
          ...
          <div className="divide-y divide-slate-700/50">
            ...
            <div className="grid grid-cols-3 p-4 bg-cyan-500/5">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-bold ${v.price === lowestPrice ? 'text-green-400' : 'text-white'}`}>
                  {formatPrice(v.price)}
            ...
            <div className="grid grid-cols-3 p-4">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className="text-center text-white">{getBodyTypeDisplay(v.bodyType)}</div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 p-4 bg-slate-800/20">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-medium ${v.seats === highestSeats ? 'text-green-400' : 'text-white'}`}>
                  {v.seats} passengers
            ...
            <div className="grid grid-cols-3 p-4">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className="text-center text-white">{getPowertrainDisplay(v.powertrain)}</div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 p-4 bg-slate-800/20">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-medium ${v.features.includes('awd') || v.features.includes('awd-available') ? 'text-green-400' : 'text-slate-500'}`}>
            ...
            <div className="grid grid-cols-3 p-4">
              ...
              {vehicles.map((v) => (
                <div key={v.name} className={`text-center font-medium ${v.features.includes('towing') || v.features.includes('heavy-towing') ? 'text-green-400' : 'text-slate-500'}`}>
