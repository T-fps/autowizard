...
export default function BestTrucksPage() {
  const trucks = sortByPopularity(vehicleDatabase.filter(v => v.bodyType === 'truck'));
  
  const midsizeTrucks = sortByPopularity(trucks.filter(v => v.size === 'midsize' || v.price < 45));
  const fullsizeTrucks = sortByPopularity(trucks.filter(v => v.size === 'fullsize' || v.price >= 45));
  const electricTrucks = sortByPopularity(trucks.filter(v => v.powertrain === 'ev'));
  const offroadTrucks = sortByPopularity(trucks.filter(v => v.features.includes('offroad')));
