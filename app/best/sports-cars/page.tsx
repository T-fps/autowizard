...
export default function BestSportsCarsPage() {
  const sportsCars = sortByPopularity(vehicleDatabase.filter(v => v.bodyType === 'sports' || v.bodyType === 'coupe' || v.bodyType === 'convertible' || v.segment === 'exotic'));
  const affordableSports = sortByPopularity(sportsCars.filter(v => v.price < 60));
  const premiumSports = sortByPopularity(sportsCars.filter(v => v.price >= 60 && v.price < 150));
  const supercars = sortByPopularity(sportsCars.filter(v => v.price >= 150 && v.price < 500));
  const hypercars = sortByPopularity(sportsCars.filter(v => v.price >= 500));
