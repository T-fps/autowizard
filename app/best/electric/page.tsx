...
export default function BestElectricPage() {
  const evs = sortByPopularity(vehicleDatabase.filter(v => v.powertrain === 'ev'));
  const affordableEvs = sortByPopularity(evs.filter(v => v.price <= 50));
  const luxuryEvs = sortByPopularity(evs.filter(v => v.segment === 'luxury' || v.segment === 'premium'));
  const evSuvs = sortByPopularity(evs.filter(v => v.bodyType === 'suv' || v.bodyType === 'crossover'));
  const evTrucks = sortByPopularity(evs.filter(v => v.bodyType === 'truck'));
